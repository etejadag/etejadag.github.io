...
class Scatter extends Chart {
  ...
  addScales() {
    ...
    const xExtent = this.dataFormat === 'file' ? extent(this.data, d => +d[this.x]) : extent(this.data[this.x]);
    const xRange = xExtent[1] - xExtent[0];
    const yExtent = this.dataFormat === 'file' ? extent(this.data, d => +d[this.y]) : extent(this.data[this.y]);
    const yRange = yExtent[1] - yExtent[0];

    const colorExtent = this.dataFormat === 'file' ? extent(this.data, d => d[this.colorVar]) : [1, 1];
    ...
    this.xScale = scaleLinear().range([0, this.width]).domain([xExtent[0] - xRange * 0.05, xExtent[1] + xRange * 0.05]);

    this.yScale = scaleLinear().range([this.height, 0]).domain([yExtent[0] - yRange * 0.05, yExtent[1] + yRange * 0.05]);
  ...
  addAxes() {
    const xAxis = axisBottom(this.xScale).tickSize(0).tickFormat(d => {
      return this.xValueFormat ? format(this.xValueFormat)(d) : d;
    });

    const yAxis = axisLeft(this.yScale).tickSize(0).tickFormat(d => {
  ...
  makeAxesRough(roughSvg, rcAxis) {
    ...
    select(`.${xAxisClass}`).selectAll('path.domain').each(function (d, i) {
    ...
    select(`.${yAxisClass}`).selectAll('path.domain').each(function (d, i) {
      const pathD = select(this).node().getAttribute('d');
  ...
  addInteraction() {
    ...
    const circles = selectAll(this.interactionG).data(this.dataFormat === 'file' ? this.data : this.data.x).append('circle').attr('cx', (d, i) => {
    ...
    selectAll(this.interactionG).on('mouseover', function () {
      ...
      that.highlight === undefined ? select(this).selectAll('path:nth-child(1)').style('opacity', 0.4) : select(this).selectAll('path:nth-child(1)').style('stroke', that.highlight);
