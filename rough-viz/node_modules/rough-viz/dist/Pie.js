...
class Pie extends Chart {
  ...
  addInteraction() {
    ...
    selectAll(this.interactionG).append('g').attr('transform', `translate(${this.width / 2}, ${this.height / 2})`).data(this.dataFormat === 'object' ? this.makePie(this.data[this.values]) : this.makePie(this.data)).append('path').attr('d', this.makeArc).attr('stroke-width', '0px').attr('fill', 'transparent');
    ...
    selectAll(this.interactionG).on('mouseover', function () {
      ...
      that.highlight === undefined ? select(this).selectAll('path').style('opacity', 0.5) : select(this).selectAll('path').style('stroke', that.highlight);
    });
  ...
  drawFromObject() {
    ...
    this.arcs.forEach((d, i) => {
      if (d.value !== 0) {
        const node = this.rc.arc(this.width / 2, // x
        this.height / 2, // y
        2 * this.radius, // width
  ...
  drawFromFile() {
    ...
    this.makePie = pie().value(d => d[this.values]).sort(null);
