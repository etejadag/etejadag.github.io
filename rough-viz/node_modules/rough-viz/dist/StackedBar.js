...
class StackedBar extends Chart {
  ...
  addScales() {
    this.xScale = scaleBand().rangeRound([0, this.width]).padding(this.padding).domain(this.data.map(d => d[this.labels]));

    this.data.sort(function (a, b) {
      return b.total - a.total;
    });
    this.yScale = scaleLinear().rangeRound([this.height, 0]).domain([0, max(this.data, d => {
      return d.total;
    })]).nice();

    // set the colors
    const keys = this.dataFormat === 'object' ? this.data.map(d => d[this.labels]) : this.data.columns;
    this.zScale = scaleOrdinal().range(['#98abc5', '#8a89a6', '#7b6888', '#6b486b', '#a05d56', '#d0743c', '#ff8c00']).domain(keys);
  }
  ...
  addInteraction() {
    ...
    .each(function (d, i) {
      ...
      select(this).append('rect').attr('x', attr['x'].value).attr('y', attr['y'].value).attr('width', attr['width'].value).attr('height', attr['height'].value).attr('fill', 'transparent');
    });
