...
class Bar extends Chart {
  ...
  addScales() {
    ...
    this.xScale = scaleBand().rangeRound([0, this.width]).padding(this.padding).domain(this.dataFormat === 'file' ? this.data.map(d => d[that.labels]) : this.data[that.labels]);

    this.yScale = scaleLinear().rangeRound([this.height, 0]).domain(this.dataFormat === 'file' ? [0, max(this.data, d => +d[that.values])] : [0, max(this.data[that.values])]);
  }
  ...
  addInteraction() {
    ...
    selectAll(this.interactionG).data(this.dataFormat === 'file' ? this.data : this.data.values).append('rect').attr('x', (d, i) => {
      return this.dataFormat === 'file' ? this.xScale(d[this.labels]) : this.xScale(this.data[this.labels][i]);
    }).attr('y', (d, i) => {
      return this.dataFormat === 'file' ? this.yScale(+d[this.values]) : this.yScale(this.data[this.values][i]);
    }).attr('width', this.xScale.bandwidth()).attr('height', (d, i) => {
      return this.dataFormat === 'file' ? this.height - this.yScale(+d[this.values]) : this.height - this.yScale(this.data[this.values][i]);
  ...
  drawFromObject() {
    ...
    this.data.values.forEach((d, i) => {
      const node = this.rc.rectangle(this.xScale(this.data[this.labels][i]), this.yScale(+d), this.xScale.bandwidth(), this.height - this.yScale(+d), {
  ...
  drawFromFile() {
    ...
    // Add barplot
    this.data.forEach(d => {
      const node = this.rc.rectangle(this.xScale(d[this.labels]), this.yScale(+d[this.values]), this.xScale.bandwidth(), this.height - this.yScale(+d[this.values]), {
        simplification: this.simplification,
        fillWeight: this.fillWeight
      });
      const roughNode = this.roughSvg.appendChild(node);
