...
class Line extends Chart {
  ...
  addScales() {
    ...
    this.xScale = this.x === undefined ? scalePoint().range([0, this.width]).domain([...Array(xExtent[1]).keys()]) : scalePoint().range([0, this.width]).domain(this.x);

    this.yScale = scaleLinear().range([this.height, 0]).domain([0, yExtent[1] + yRange * 0.05]);
  }
  ...
  addInteraction() {
    ...
    this.dataSources.map((key, idx) => {
      ...
      const lineGen = line().x(d => d[0]).y(d => d[1]);
    ...
    const mousemove = function (d) {
      ...
      that.dataSources.map((key, i) => {
        ...
        if (that.dataFormat === 'file') {
          select(textClass).selectAll('text').style('opacity', 1).html(that.x === undefined ? `(${xSpot},${hoverData[key]})` : `(${that.x[xSpot]}, ${hoverData[key]})`).attr('x', that.x === undefined ? that.xScale(xSpot) : that.xScale(that.x[xSpot])).attr('y', that.yScale(hoverData[key]) - 6);
        } else {
          select(textClass).selectAll('text').style('opacity', 1).html(that.x === undefined ? `(${xSpot}, ${hoverData})` : `(${that.x[xSpot]}, ${hoverData})`).attr('x', that.x === undefined ? that.xScale(xSpot) : that.xScale(that.x[xSpot])).attr('y', that.yScale(hoverData));
        }
      });
    };
