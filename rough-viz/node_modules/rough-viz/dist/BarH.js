...
class BarH extends Chart {
  ...
  addScales() {
    const that = this;
    this.yScale = scaleBand().rangeRound([0, this.height]).padding(this.padding).domain(this.dataFormat === 'file' ? this.data.map(d => d[that.labels]) : this.data[that.labels]);

    this.xScale = scaleLinear().rangeRound([0, this.width]).domain(this.dataFormat === 'file' ? [0, max(this.data, d => +d[that.values])] : [0, max(this.data[that.values])]);
  }
  ...
  addInteraction() {
    ...
    selectAll(this.interactionG).data(this.dataFormat === 'file' ? this.data : this.data.values).append('rect').attr('x', 0).attr('y', (d, i) => {
