...
class Line extends _Chart.default {
  ...
  addOverlayX() {
    ...
    this.overlayX.forEach((item, i) => {
      this.svg.append('text').attr('x', i * this.width / (this.overlayX.length - 0.8)).attr('y', this.height + 50).style('font-size', this.xAxisFontSize === undefined ? `${Math.min(20, Math.min(this.width, this.height) / 4)}px` : this.overlayXFontSize).style('font-family', this.fontFamily).style('opacity', 0.8).text(item);
    });
  }
  makeAxesRough(roughSvg, rcAxis) {
    const xAxisClass = `xAxis${this.graphClass}`;
