    ...
    const mousemove = function (d) {
      ...
      that.dataSources.map((key, i) => {
        ...
        if (that.dataFormat === 'file') {
          (0, _d3Selection.select)(textClass).selectAll('text').style('opacity', 1).html(that.x === undefined ? `(${xSpot},${hoverData[key]})` : `(${that.x[xSpot]}, ${hoverData[key]})`).attr('x', that.x === undefined ? that.xScale(xSpot) : that.xScale(that.x[xSpot])).attr('y', that.yScale(hoverData[key]) - 6);
        } else {
